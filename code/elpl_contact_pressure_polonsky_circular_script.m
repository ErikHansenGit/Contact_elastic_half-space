...
%% Settings
% Algorithm settings:
W_aim   = 10;                                                               % [N] imposed normal load force
...
a           = 1e-6;                                                         % [m] roughness amplitude
lambda_x1   = (geo.x1(geo.Nx1) - geo.x1(1) + geo.dx1)/3;                              % [m] roughness wavelengh in x1-direction
lambda_x2   = (geo.x2(geo.Nx2) - geo.x2(1) + geo.dx2)/0.5;                  % [m] roughness wavelengh in x2-direction
...
[fft2_Kernel_circ]  = construct_circular_Kernel(geo.Nx1,geo.Nx2,geo.dx1,geo.dx2,sld.E_dash);

% Check limits for normal load:  
W_min = geo.Nx1*geo.dx1*geo.Nx2*geo.dx2*p_hd;
W_max = geo.Nx1*geo.dx1*geo.Nx2*geo.dx2*sld.H;
if W_aim <= W_min
    fprintf('Warning!\n')
    fprintf('W_aim <= W_min = %d\n',W_min)
    fprintf('h_s is not unambigous since there is no surface contact!\n')
elseif W_aim >= W_max
    fprintf('Warning!\n')
    fprintf('W_aim >= W_max = %d\n',W_max)
    fprintf('h_s is not unambigous since there is plastic deformation in each grid point!\n')
end

% Compute pressure field:  
[p_con,g,err] = elpl_contact_pressure_polonsky_circular(p_hd,sld.H,geo.prof,W_aim,geo.Nx1,geo.Nx2,geo.dx1,geo.dx2,fft2_Kernel_circ,err_tol,it_max,h_ref);
% Compute resulting normal load:
W = geo.dx1*geo.dx2*sum(p_con(:));

% Compute deformations and deformed profiles:
[h_el] = compute_h_el_circular(p_con,geo.Nx1,geo.Nx2,fft2_Kernel_circ);                   % [m] elastic deformation
clear fft2_Kernel;
z_el = geo.prof - h_el;                                                     % [m] elastically deformed profile
A_el = find(p_con>p_hd & p_con<sld.H);                                      % [-] indices of elastically deformed points
if  isempty(A_el)
    fprintf('Warning!\n')
    fprintf('h_s is not unambigous since there is plastic deformation in each contact grid point!\n')
end
h_s = mean(z_el(A_el));                                                     % [m] seperating distance between upper and lower reference planes
clear A_el;
...
% W_aim             [N]     imposed normal load force
% Nx1               [-]     number of points in x1-direction
% Nx2               [-]     number pf points in x2-direction
% dx1               [m]     cell length in x1-direction
% dx2               [m]     cell length in x2-direction
...
[u] = compute_h_el_circular(p_con,Nx1,Nx2,fft2_Kernel_circ);  % [m] elastic deformation 
...
% Find indices of points that are in the non-contact and plastic
...
while i_it == 0 || (err(i_it)>err_tol && i_it<it_max)
    ...
    [t_conv]    = compute_h_el_circular(t,Nx1,Nx2,fft2_Kernel_circ);     
    r           = -t_conv;
    clear t_conv;   
    ...
    % Update pressure and confine it to the upper and lower limit:
    p_con(A_free)       = p_con(A_free) - tau*t(A_free);
    p_con(p_con<p_min)  = p_min;
    p_con(p_con>H)      = H;
    
    % Rescale the pressure in the free domain in such a way that the 
    % desired normal load W_aim is met by the whole pressure profile:
    A_tot               = linspace(1,Nx1*Nx2,Nx1*Nx2)';
    A_unscalable        = setdiff(A_tot,A_free);
    W_scalable          = sum(p_con(A_free))*dx1*dx2;
    W_unscalable        = sum(p_con(A_unscalable))*dx1*dx2;
    p_con(A_free)       = (W_aim - W_unscalable)/W_scalable*p_con(A_free);
    clear A_tot; clear A_unscalable; clear W_scalable; clear W_unscalable;
...
function [h_el] = compute_h_el_circular(p,Nx1,Nx2,fft2_Kernel_circ)
% Computes elastic displacement due to pressure field with circular
